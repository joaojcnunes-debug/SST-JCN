...
import { useQpsPlanoMedidas, useQpsSalvarPlanoMedidas } from "@/lib/hooks/useQpsGestao";
import { useCanEdit } from "@/lib/hooks/useUsuario";
import { MEDIDAS_CONTROLE, MESES } from "@/lib/drps/topicos";
import type { QpsPlanoMedidas } from "@/lib/supabase/types";

/**
 * Plano Anual de Medidas de Controle — espelho de `psicossocial/[idRelatorio]/medidas`
 * (v225, "igual ao DRPS"). Os 13 programas são os mesmos do DRPS (`MEDIDAS_CONTROLE`),
 * de propósito: é o catálogo da JCN Consultoria, não do instrumento.
 */

type MedidaPlano = QpsPlanoMedidas["plano"][string];
...
export default function MedidasQpsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const canEdit = useCanEdit();
  const { data: ap } = useQpsAplicacao(id);
  const anoAtual = new Date().getFullYear();
  const [ano, setAno] = useState(anoAtual);
  const { data: planoDB, isLoading } = useQpsPlanoMedidas(id, ano);
  const salvar = useQpsSalvarPlanoMedidas();
  ...
  function onSalvar() {
    if (!ap) return;
    salvar.mutate({ id_aplicacao: id, ano, plano }, { onSuccess: () => setDirty(false) });
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold text-gray-900">Medidas de Controle — Plano Anual</h1>
        <p className="text-sm text-gray-600">
          {ap?.titulo ?? "Carregando..."} · calendário de implementação dos 13 programas recomendados.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
        <div className="flex items-center gap-3">
          <label className="text-xs font-semibold uppercase tracking-wider text-gray-500">Ano:</label>
          <select
            value={ano}
          ...
          <span className="text-xs text-gray-500">{totalMarcacoes} marcação(ões)</span>
        </div>
        <button
          type="button"
          onClick={onSalvar}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-gray-50 text-[10px] uppercase text-gray-500">
              <tr>
                <th className="px-2 py-2 text-left font-medium sticky left-0 bg-gray-50">Ação</th>
                {MESES.map((m) => (
              ...
              {MEDIDAS_CONTROLE.map((acao) => {
                ...
                return (
                  <tr key={acao} className="hover:bg-gray-50">
                    <td className="px-2 py-1 font-medium text-gray-800 sticky left-0 bg-white">{acao}</td>
                    {p.meses.map((m, i) => (
                    ...
                    <td className="px-2 py-1">
                      <input
                        type="text"
                        value={p.responsavel}
                        onChange={(e) => setResponsavel(acao, e.target.value)}
                        disabled={!canEdit}
                        placeholder="—"

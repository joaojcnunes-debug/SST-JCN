...
import { useQpsAplicacao, useQpsRespondentes } from "@/lib/hooks/useQuestionarios";
import { listarSetoresQps, TODOS_OS_SETORES } from "@/lib/qps/gravidade";
import {
  useQpsPlanoAcao5w2h,
  useSalvarLinhaQpsPlanoAcao,
  useRemoverLinhaQpsPlanoAcao,
} from "@/lib/hooks/useQpsGestao";
import type { QpsPlanoAcao5w2h, StatusQpsPlanoAcao5w2h } from "@/lib/supabase/types";
import {
  useAcaoOque,
  useAcaoComo,
  comoOpcoesDeTitulo,
...
/**
 * Plano de Ação 5W2H da aplicação — espelho de `psicossocial/[idRelatorio]/plano-acao`
 * (v225, "igual ao DRPS"). O catálogo "O quê → Como" é o mesmo do DRPS (é global).
 * O que muda: id_aplicacao no lugar de id_relatorio e os setores vêm dos
 * respondentes do questionário. `MultiSelectSetores` e `Campo` são cópias dos
 * helpers privados da tela do DRPS.
 */

const STATUS_OPCOES: { valor: StatusQpsPlanoAcao5w2h; label: string; classe: string }[] = [
...
export default function PlanoAcaoQpsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const canEdit = useCanEdit();

  const { data: ap, isLoading: loadAp } = useQpsAplicacao(id);
  const { data: linhas = [], isLoading: loadLinhas } = useQpsPlanoAcao5w2h(id);
  const { data: respondentes = [], isLoading: loadResp } = useQpsRespondentes(id);
  const { data: oques = [] } = useAcaoOque();
  const { data: comos = [] } = useAcaoComo();
  const salvar = useSalvarLinhaQpsPlanoAcao();

  // Setores avaliados nesta aplicação (origem dos respondentes) p/ o select "Onde".
  const setores = listarSetoresQps(respondentes);

  function handleAdicionar() {
    if (!ap || !canEdit) return;
  ...
  if (loadAp || loadLinhas || loadResp) return <EditorSkeleton />;
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">
          {ap.titulo} · ações de gerenciamento dos riscos psicossociais no formato 5W2H
          (O quê, Por quê, Onde, Quando, Quem, Como, Quanto custa). Compõe um capítulo
          próprio do laudo da {INSTRUMENTO_QAP.sigla}.
        </p>
      </div>
...
function LinhaCard({
  ...
  const salvar = useSalvarLinhaQpsPlanoAcao();
  const remover = useRemoverLinhaQpsPlanoAcao();
  ...
  const [form, setForm] = useState({
    ...
    status: (linha.status ?? "PENDENTE") as StatusQpsPlanoAcao5w2h,
  });
  ...
  const opcoesOnde = Array.from(
    new Set([TODOS_OS_SETORES, ...setores, ...selecionadosOnde].filter(Boolean)),
  );
      ...
      {canEdit && (
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-gray-100 pt-3">
          <label className="flex items-center gap-2 text-xs text-gray-600">
            ...
            <select
              value={form.status}
              onChange={(e) => set("status", e.target.value as StatusQpsPlanoAcao5w2h)}
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => remover.mutate({ id: linha.id, id_aplicacao: idAplicacao })}
...
/** Dropdown de múltipla escolha (checkboxes, fecha ao clicar fora) — cópia do DRPS. */
function MultiSelectSetores({
  ...
  const label = selecionados.size === 0 ? "Selecione…" : Array.from(selecionados).join(", ");

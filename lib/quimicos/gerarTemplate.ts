...
export function gerarConclusaoTemplate(
  ...
  // --- Campos narrativos (templates por forma física + flags) ---
  const epiBase = EPI_POR_FORMA[forma] || EPI_POR_FORMA["Líquido"];
  const epi_necessarios = ajustarEpiPorFlags(epiBase, d.pele, carcinog);
  const epc_necessarios = EPC_POR_FORMA[forma] || EPC_POR_FORMA["Líquido"];
  const emergencia_acidente = EMERGENCIA_POR_FORMA[forma] || EMERGENCIA_POR_FORMA["Líquido"];
  ...
  const metodologia =
    METODOLOGIA_POR_ANEXO[chaveAnexo] || METODOLOGIA_POR_ANEXO["Anexo 11"];
  const como_medir =
  ...
  if (d.grau_nr15 && d.anexo) {
    resumoParts.push(
      `Conforme NR-15 ${d.anexo}, o agente${ehMistura ? " (mistura)" : ""} está classificado com grau ${d.grau_nr15.toLowerCase()} de insalubridade${
        d.lt_ppm != null
  ...
  if (medicao_necessaria === "SIM") {
    conclusao.push("recomenda-se monitoramento quantitativo periódico");
  }
  conclusao.push(
    `uso obrigatório de EPIs adequados à forma física ${forma.toLowerCase()}`
  );
  if (d.pele) conclusao.push("com proteção dérmica");

...
import { categoriaInventario, type AbaInventario } from "@/lib/inventario/categorias";
...
const DESCRICAO: Record<AbaInventario, string> = {
  equipamentos: "Material interno da JCN Consultoria. Cadastre e movimente (transferências) os equipamentos do patrimônio.",
  maquinas: "Máquinas de empresas clientes cadastradas no inventário.",
  medicoes: "Instrumentos de medição (medidores, luxímetros, dosímetros, balanças, etc.).",
};

/**
 * Lista do inventário FILTRADA por uma categoria (aba do sidebar). A tela de
 * "Visão geral" (que mostra tudo com abas) permanece separada e inalterada.
 */
export default function InventarioLista({ categoria }: { categoria: AbaInventario }) {
  const canCreate = useCanCreate({ exigeEditar: true });
  const { data: maquinas = [], isLoading } = useInventarioMaquinas();
  const { data: empresas = [] } = useEmpresas();
  const [busca, setBusca] = useState("");
  const [filtroStatus, setFiltroStatus] = useState<StatusMaquina | "TODAS">("TODAS");
  ...
  const itens = useMemo(
    () => maquinas.filter((m) => categoriaInventario(m) === categoria),
    [maquinas, categoria],
  );
  ...
  const filtradas = useMemo(() => {
    ...
    return itens.filter((m) => {
      ...
      if (!q) return true;
      return [m.nome, m.marca, m.modelo, m.numero_serie, m.codigo_interno, m.tag, m.numero_patrimonio, m.localizacao]
        .filter(Boolean)
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          <ArrowLeft className="size-3.5" /> Visão geral
        </Link>
        <div className="flex items-center gap-2">
          ...
          {canCreate && (
            <Link
              href={`/inventario-maquinas/nova?cat=${categoria}`}
      ...
      <div>
        <h1 className="flex items-center gap-2 text-xl font-semibold text-gray-900">
          ...
          {CATEGORIA_INVENTARIO_LABELS[categoria]}
        </h1>
        <p className="text-sm text-gray-600">{DESCRICAO[categoria]}</p>
      </div>

      {/* Cards de resumo */}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <ResumoCard label="Total" valor={isLoading ? "…" : itens.length} icon={<Boxes className="size-4" />} cor="bg-blue-50 text-blue-700 border-blue-200" />
        <ResumoCard label="Operantes" valor={isLoading ? "…" : totais.OPERANTE} icon={<Boxes className="size-4" />} cor="bg-emerald-50 text-emerald-700 border-emerald-200" />
        <ResumoCard label="Em manutenção" valor={isLoading ? "…" : totais.MANUTENCAO} icon={<Wrench className="size-4" />} cor="bg-amber-50 text-amber-700 border-amber-200" />
        <ResumoCard label="Inativas / baixadas" valor={isLoading ? "…" : totais.INATIVA + totais.BAIXADA} icon={<CircleSlash className="size-4" />} cor="bg-gray-50 text-gray-700 border-gray-200" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          value={filtroStatus}
          onChange={(e) => setFiltroStatus(e.target.value as StatusMaquina | "TODAS")}
      ...
      <section>
        {isLoading ? (
          ...
          <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-6 text-center text-sm text-gray-500">
            {itens.length === 0 ? (
              <>
                Nenhum item em <strong>{CATEGORIA_INVENTARIO_LABELS[categoria]}</strong> por enquanto.{" "}
                {canCreate && <>Clique em <strong>Cadastrar</strong> para começar.</>}
              </>
            ) : (
              ...
              <li key={m.id_maquina}>
                ...
                >
                  <div className="flex size-16 shrink-0 items-center justify-center overflow-hidden rounded-md bg-gray-100">
                    {m.foto_url ? (
                      <StorageImg stored={m.foto_url} alt={m.nome} className="size-full object-cover" />
                    ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${STATUS_CORES[m.status]}`}>
                    ...
                    <p className="mt-0.5 truncate text-xs text-gray-500">
                      {m.id_empresa ? empresaMap.get(m.id_empresa) ?? "Empresa removida" : "Patrimônio JCN"}
...
function ResumoCard({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border bg-white p-3 shadow-sm">
      <div className={`flex size-9 items-center justify-center rounded-md ${cor}`}>{icon}</div>

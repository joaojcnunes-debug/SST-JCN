...
import { formatarDataBR, type ResumoGestao } from "@/lib/drps/gestao";

/**
 * Quadro impresso "Painel de Gestão — Resumo Executivo": indicadores das 3
 * frentes (Medidas / Monitoramento / Revisão) num quadro só. Presentacional:
 * recebe o `resumo` já calculado (`calcularResumoGestao`) — o DRPS e a QAP
 * calculam cada um com seus hooks e renderizam este mesmo quadro (v226).
 */
export default function GestaoResumoPrint({
  resumo,
  ano,
  numero,
  /** Como o texto de introdução chama o instrumento ("diagnóstico", "questionário"…). */
  instrumentoTexto = "diagnóstico",
}: {
  resumo: ResumoGestao;
  ano: number;
  numero?: number;
  instrumentoTexto?: string;
}) {
  return (

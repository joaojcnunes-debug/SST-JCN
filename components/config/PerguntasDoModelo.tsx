...
import {
  ...
  useSavePerguntaModelo,
  useDeletePerguntaModelo,
} from "@/lib/hooks/useV3";
import Modal from "@/components/ui/Modal";
import ConfirmDialog from "@/components/ui/ConfirmDialog";
import { gerarId, cn } from "@/lib/utils";
import type { PerguntaModeloRisco } from "@/lib/supabase/types";

/**
 * Lista + modal de perguntas customizadas escopadas num modelo.
 * Espelha PerguntasDoTipo, mas trabalha em perguntas_modelo_risco
 * (V5) em vez de perguntas_tipo_risco (V3).
 */
export default function PerguntasDoModelo({
  idModelo,
}: {
  idModelo: string;
}) {
  const { data: perguntas = [] } = usePerguntasDoModelo(idModelo);
  const save = useSavePerguntaModelo();
  const del = useDeletePerguntaModelo();
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<PerguntaModeloRisco | null>(null);
  const [confirm, setConfirm] = useState<PerguntaModeloRisco | null>(null);
    ...
    <>
      ...
      {perguntas.length === 0 ? (
        <div className="rounded-md border border-dashed border-gray-300 bg-white p-3 text-center text-xs text-gray-500">
          Nenhuma pergunta customizada para este modelo ainda.
        </div>
      ) : (
      ...
      <ConfirmDialog
        ...
        description={`"${confirm?.texto}" será removida deste modelo.`}
        variant="danger"
...
function PerguntaModeloModal({
  ...
  onClose: () => void;
  idModelo: string;
  editing: PerguntaModeloRisco | null;
}) {
  const save = useSavePerguntaModelo();
  const { data: existentes = [] } = usePerguntasDoModelo(idModelo);
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (!editing) {
      if (!chaveFinal) chaveFinal = slugify(form.texto);
    ...
    if (!/^[a-z][a-z0-9_]*$/i.test(chaveFinal)) {
      toast.error(
        "Não foi possível gerar uma chave válida. Informe uma manualmente."
      );
      return;
    }

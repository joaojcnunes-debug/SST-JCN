-- v203_gestao_subtarefa_modelos.sql
-- GESTAO-KANBAN-01-F2.2c — modelos de checklist de subtarefa (estende F2.2a/v201 + F2.2b/v202).
-- Uma etapa pode, por automação, criar um checklist de subtarefas a partir de um MODELO do quadro
--   (ex.: "ao entrar em Elaboração → criar as subtarefas do modelo 'PGR'"). Os 27 checklists
--   "Importação dos produtos" do Runrun viram modelos editáveis por quadro (o SEED é F2.2e/F4).
-- Estende gestao_automacao_aplicar (corpo v2 VIVO lido VERBATIM via pg_get_functiondef — já com os
--   ramos F2.2a/b; adiciona SOMENTE o ramo `criar_subtarefas_modelo`, sem tocar no resto).
-- SQL-only. Idempotente. NÃO grava schema_migrations (apply manual pelo operador + reload PostgREST).
-- Segurança: escrita direta do modelo gated pela RLS (padrão de config de quadro, como
--   gestao_formularios/gestao_automacoes) — SEM anon; a inserção de subtarefas roda dentro do
--   definer (aplicar), valores do jsonb parametrizados, zero SQL dinâmico; search_path='public' fixo.

begin;

-- ============================================================================
-- 1. Tabela gestao_subtarefa_modelos (idempotente)
-- ============================================================================
create table if not exists public.gestao_subtarefa_modelos (
  id         uuid primary key default gen_random_uuid(),
  id_quadro  text not null references public.gestao_quadros(id_quadro) on delete cascade,
  slug       text not null,
  titulo     text not null,
  itens      jsonb not null default '[]'::jsonb,  -- array de {texto, etapa?, tipo?}
  created_at timestamptz default now(),
  unique (id_quadro, slug)
);

create index if not exists gestao_subtarefa_modelos_quadro_idx on public.gestao_subtarefa_modelos (id_quadro);

-- ============================================================================
-- 2. Grants + RLS (lição default-privileges: fechar tudo, abrir só o necessário)
--    Espelha gestao_automacoes: authenticated com escrita direta gated pela RLS; SEM anon.
-- ============================================================================
alter table public.gestao_subtarefa_modelos enable row level security;

revoke all on public.gestao_subtarefa_modelos from authenticated, anon, public;
grant select, insert, update, delete on public.gestao_subtarefa_modelos to authenticated;
-- backup_operator=r: cobre a tabela nova no dump logico (mesmo motivo da F2.1/gestao_subtarefas).
grant select on public.gestao_subtarefa_modelos to backup_operator;

drop policy if exists gestao_subtarefa_modelos_sel on public.gestao_subtarefa_modelos;
create policy gestao_subtarefa_modelos_sel on public.gestao_subtarefa_modelos
  for select to authenticated
  using (public.gestao_pode_ver(id_quadro));

drop policy if exists gestao_subtarefa_modelos_wr on public.gestao_subtarefa_modelos;
create policy gestao_subtarefa_modelos_wr on public.gestao_subtarefa_modelos
  for all to authenticated
  using (public.gestao_pode_editar_q(id_quadro))
  with check (public.gestao_pode_editar_q(id_quadro));

-- ============================================================================
-- 3. gestao_automacao_aplicar — corpo v2 VIVO VERBATIM (F2.2a/v201 + F2.2b/v202)
--    + SOMENTE o ramo criar_subtarefas_modelo (inserido antes do `else` final).
--    Novos locais (v_modelo/v_item/v_ordem/v_txt) declarados sob "extensões F2.2c".
-- ============================================================================

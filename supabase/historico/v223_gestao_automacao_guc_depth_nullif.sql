-- v223_gestao_automacao_guc_depth_nullif.sql
-- GE3 esteira -- corrige erro intermitente "invalid input syntax for type integer: ''" no motor.
-- current_setting(GUC customizado, true) retorna STRING VAZIA (nao NULL) quando o placeholder
-- ja foi registrado na conexao do pool e nao esta setado; coalesce(...,'0') nao troca ''
-- -> ''::int estoura e reverte a acao (ex.: mover_status parou o salto Inspecao Pendente->na Pasta).
-- Fix: nullif(current_setting(...),'') no read de gestao.autom_depth em run/aplicar/prazos.
-- Recria as 3 funcoes VERBATIM (pg_get_functiondef) trocando SO o read. Regressao zero.
...
begin
  ...
  v_depth := coalesce(nullif(current_setting('gestao.autom_depth', true), ''), '0')::int;
  if v_depth >= 3 then

-- v149 (JCN): DRPS — captura da "unidade de trabalho" informada no Forms.
-- Reflete a v137 do painel-sst.
--
-- Contexto: o formulário de alguns clientes tem uma pergunta demográfica a mais
-- ("Qual sua unidade de trabalho?", coluna 2 de 54). O parser já tolera essa
-- coluna extra ancorando as respostas no fim da linha — mas apenas ABSORVIA a
-- coluna: lia e descartava. Esta migration cria o destino do dado; o parser
-- passa a preenchê-lo na mesma versão.
--
-- FASE 1 (esta): só captura. Nada lê a coluna ainda, nenhum cálculo muda,
-- nenhum laudo existente é afetado. Relatórios cujo formulário não tem a
-- pergunta ficam com NULL e seguem funcionando exatamente como hoje.
-- FASE 2 (v150): unidade no filtro / hierarquia no laudo.
...
-- Não confundir com a tabela public.unidades, que é um agrupamento de empresas
-- para controle de acesso — conceito diferente, nível acima da empresa. Esta
-- coluna é o local/posto de trabalho DENTRO de uma empresa, texto livre vindo
...
-- Conferência: a coluna nova tem de existir e ser nullable.
do $$
declare
  v_existe boolean;
begin

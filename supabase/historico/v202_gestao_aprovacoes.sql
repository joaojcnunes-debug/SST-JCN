-- v202_gestao_aprovacoes.sql
-- GESTAO-KANBAN-01-F2.2b — aprovação no motor v2 (estende F2.2a/v201).
-- Fluxo: ação de automação `solicitar_aprovacao` cria uma pendência; o aprovador decide
--   pela Caixa de Entrada (RPC gestao_decidir_aprovacao); ao aprovar, dispara o gatilho
--   `tarefa_aprovada` (o run v2 já sabe iterar por gatilho — nada muda no run).
-- Estende gestao_automacao_aplicar (corpo v2/v201 lido VERBATIM via pg_get_functiondef;
--   adiciona SOMENTE o ramo `solicitar_aprovacao`, sem tocar no resto).
-- SQL-only. Idempotente. NÃO grava schema_migrations (apply manual pelo operador + reload PostgREST).
-- Segurança: escrita só por RPC SECURITY DEFINER (sem INSERT/UPDATE direto de authenticated);
--   RLS SELECT gated; EXECUTE do RPC só a authenticated; search_path='public' fixo; zero SQL dinâmico.

begin;

-- ============================================================================
-- 1. Tabela gestao_aprovacoes (idempotente)
-- ============================================================================
create table if not exists public.gestao_aprovacoes (
...
create index if not exists gestao_aprovacoes_tarefa_idx    on public.gestao_aprovacoes (id_tarefa);
create index if not exists gestao_aprovacoes_aprovador_idx on public.gestao_aprovacoes (aprovador_email) where status = 'pendente';

-- ============================================================================
-- 2. Grants + RLS (lição default-privileges: fechar tudo, abrir só o necessário)
--    Espelha gestao_tarefa_vinculados: authenticated só SELECT (via RLS); escrita só por RPC.
-- ============================================================================
alter table public.gestao_aprovacoes enable row level security;

revoke all on public.gestao_aprovacoes from authenticated, anon, public;
grant select on public.gestao_aprovacoes to authenticated;

drop policy if exists gestao_aprovacoes_sel on public.gestao_aprovacoes;
create policy gestao_aprovacoes_sel on public.gestao_aprovacoes
  for select to authenticated
  using (public.gestao_ve_tarefa(id_tarefa) or lower(aprovador_email) = public.gestao_email());

-- Sem policy de INSERT/UPDATE/DELETE para authenticated: toda escrita passa pelo RPC (definer).

-- ============================================================================
-- 3. RPC gestao_decidir_aprovacao — aprovador designado OU gestor decide;
--    ao aprovar, dispara o gatilho tarefa_aprovada (run v2 itera por gatilho).
-- ============================================================================
create or replace function public.gestao_decidir_aprovacao(p_id uuid, p_decisao text, p_motivo text default null)
  returns void
  language plpgsql
  security definer
...
revoke all on function public.gestao_decidir_aprovacao(uuid, text, text) from public, anon;
grant execute on function public.gestao_decidir_aprovacao(uuid, text, text) to authenticated;

-- ============================================================================
-- 4. gestao_automacao_aplicar — corpo v2/v201 VERBATIM + SOMENTE o ramo solicitar_aprovacao
--    (inserido imediatamente antes do `else` final; nada mais muda).
-- ============================================================================

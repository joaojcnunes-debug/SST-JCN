-- v152 (JCN) — Inventário: unidade como ID (seletor) + transferência com aceite assinado.
-- Reflete a v152 do painel-sst.
--
-- Diferença em relação ao painel, de propósito: unaccent e pg_trgm são
-- instaladas no schema `extensions` (padrão deste projeto, onde já moram
-- pgcrypto e uuid-ossp) e as chamadas do backfill são qualificadas.
--
-- O QUE MUDA
...
begin
  if to_regclass('public.unidades') is null then
    raise exception 'v152 abortada: tabela public.unidades nao existe';
  end if;
  if to_regproc('public.caller_unidades') is null or to_regproc('public.caller_eh_admin') is null then
    raise exception 'v152 abortada: funcoes de acesso (v75) ausentes';
  end if;
end $$;
...
update public.inventario_maquinas m
 ...
 where m.id_unidade is null
   ...
   and (
     lower(extensions.unaccent(btrim(m.unidade))) = lower(extensions.unaccent(btrim(u.nome)))          -- acento/caixa
     or extensions.similarity(lower(extensions.unaccent(m.unidade)), lower(extensions.unaccent(u.nome))) > 0.6    -- erro de digitação
   );
...
-- ── 5) RLS do inventário por unidade: NÃO APLICADA AQUI, de propósito ────────
-- O painel isola o equipamento interno por unidade com uma policy
-- `inventario_maquinas_sel_uni`, que pressupõe a v76 dele ("RLS por unidade"),
-- onde TODA policy antiga foi substituída pelo par <tabela>_sel_uni/_rw_uni.
--
-- O JCN nunca recebeu esse modelo: aqui todas as tabelas seguem com
-- `auth read <tabela>` USING (true) e `auth write <tabela>`. Criar a policy do
-- painel neste estado não isolaria nada — policies permissivas se somam com OR,
-- e o OR com `true` engole qualquer restrição. Seria uma trava que parece
-- existir e não existe, que é pior do que não ter.
--
-- Ligar o isolamento de verdade exigiria derrubar a `auth read
-- inventario_maquinas` e deixar esta tabela como a única do sistema com regra
-- restritiva — uma mudança do modelo de acesso do JCN, não um reflexo do painel.
-- Fica registrado aqui como decisão consciente; o dia que o JCN adotar o modelo
-- por unidade, esta seção volta junto.

-- v203_gestao_subtarefa_modelos.rollback.sql
-- Reverte v203: repõe gestao_automacao_aplicar SEM o ramo criar_subtarefas_modelo (corpo v2 VIVO
--   VERBATIM — já com os ramos F2.2a/v201 + F2.2b/v202) e remove a tabela gestao_subtarefa_modelos.
-- ATENÇÃO: só rode se nenhuma fase POSTERIOR (que também estenda `aplicar`) estiver aplicada; caso
...
-- 1. gestao_automacao_aplicar — corpo v2 VERBATIM (F2.2a + F2.2b), sem o ramo criar_subtarefas_modelo.
...
-- 2. Remove a tabela (índice e policies caem junto com a tabela).
drop table if exists public.gestao_subtarefa_modelos;

-- 3. Baixa do registro de migração (se tiver sido gravado no apply manual).
delete from schema_migrations where version = 'v203_gestao_subtarefa_modelos';
